...
$st = $pdo->prepare('SELECT * FROM usuario u  left join profissional  p on(  p.identificacao_anucio = u.id) left join imagem im on(p.identificacao_anucio = im.identificacao_imagem) WHERE qualificacao LIKE :qualificacao  AND ambiente = :ambiente AND localatendimento = :localatendimento  AND especial = :especial');
