...
function valida() {
   ...
   $pdo = new Connection();
    ...
    $sql2 = "SELECT * FROM usuario u left join notificacao n  on (u.id = n.identificacao_notifcacao) left join cliente_profissional cp on(u.id = cp.id) left join profissional  p on( cp.identificacao = p.id )  left join imagem_perfil im_p on(u.id = im_p.identificacao_perfil)  where cpf like :cpf and  senha = :senha ";
    ...
    $stmtc->execute();
     ...
     $_SESSION['usuarioimagens'] =  $reg2["imagens"];
     $_SESSION['usuarioidentificacao_perfil'] =  $reg2["identificacao_perfil"];
